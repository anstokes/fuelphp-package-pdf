...
class Pdf_Table
{
     ...
     *
     */
    protected $aTableHeaderType = [];

    /**
     * Header is drawn or not
     *
     * @var boolean
     */
     ...
     */
    public function initialize(array $aColumnWidths, $aConfiguration = [])
    {
     ...
     */
    public function addHeader($aHeaderRow = [])
    {
    ...
    public function setHeaderProperty($nColumn, $sPropertyKey, $sPropertyValue, $nRow = 0)
    {
        for ($i = 0; $i <= $nRow; $i++) {
            if (!isset($this->aTableHeaderType[ $i ])) {
                $this->aTableHeaderType[ $i ] = [];
            }
        }

        if (!isset($this->aTableHeaderType[ $nRow ][ $nColumn ])) {
            $this->aTableHeaderType[ $nRow ][ $nColumn ] = [];
        }
     ...
     */
    public function addRow($aRowData = [])
    {
