...
class Pdf_Multicell
{
    ...
    public function setStyle($tag, $fontfamily, $fontstyle, $fontsize, $color)
    {
        ...
        $this->TagStyle[ $tag ][ 'color' ] = is_string($color) ? trim($color) : $color;
    }
    ...
    protected function makeLine($nWidth)
    {
        ...
        foreach ($aDataInfo as $key => $val) {
            ...
            while ($i < $nStringLength) {
                ...
                $char_width = $this->mtGetCharWidth($tag, $c);
                    ...
                    if ($last_sep != -1) {
                        ...
                        if ($last_sepch == ord(" ")) { //the last tag ends with a space, have to remove it
                            ...
                            if (' ' == self::strchar($temp[ 'text' ], -1)) {
                                ...
                                $temp[ 'width' ] -= $this->mtGetCharWidth($temp[ 'tag' ], ord(' '));
                                $temp[ 'spaces' ]--;
    ...
    public function multiCell(
        ...
        $w_text = $this->mtGetAvailableTextWidth($nWidth, $nPaddingLeft, $nPaddingRight);
    ...
    protected function printLine($nWidth, $nHeight, $aTxt, $align = 'J')
    {
        ...
        foreach ($aTxt as $val) {
            ...
            $sCellCode = $this->oPdf->getCellCodePublic($width, $nHeight, $val[ 'text' ], self::DEBUG_CELL_BORDERS, 0, "C", 0, $val[ 'href' ]);
            ...
            if ($align == 'J') {
                ...
                if (!$this->oPdf->isUnicodeFont()) {
                    ...
                    $this->oPdf->_outPublic(sprintf('%.3f Tw', $temp_X * $this->oPdf->k()));
                } else {
                    if ($val[ 'spaces' ] > 0) {
                ...
                if (!$this->oPdf->isUnicodeFont()) {
                    $this->oPdf->ws = 0;
                    $this->oPdf->_outPublic('0 Tw');
                }
            }

            $this->oPdf->_outPublic($sCellCode);
     ...
     */
    protected function mtGetCharWidth($tag, $char)
    {
     ...
     */
    protected function mtGetAvailableTextWidth($nWidth, $nPaddingLeft = 0, $nPaddingRight = 0)
    {
        //if with is == 0
        if (0 == $nWidth) {
            $nWidth = $this->oPdf->w() - $this->oPdf->rMargin() - $this->oPdf->x();
